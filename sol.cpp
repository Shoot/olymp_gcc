    ...
    */
    void solve() {
        ...
        vector<vpll> sm(n);
        for (ll i = 0; i < n; i += 1) {
            for (ll k = -K; k <= K; k += 1) if (i+k >= 0 && i+k < n && !never[i+k]) {
                sm[i].push_back(make_pair(a[i+k], k));
            }
        }
        set<pll> st;
        const ll inf = 1e10;
        ...
        cout << endl;
    //    cout << par[n-1].first << " " << par[n-1].second << endl;
    }
