...
//        cout << x << " " << xtl << " " << xtr << " " << ytl << " " << ytr << " " << gtval(ux) << endl;
        assert(ux != -1);
        assert(uy != -1);
        if (xtl == xtr) {
...
//    cout << gt(gt, roots[0], 1, n, -1e9, 1e9, 1, 5, 0, 0) << "!!!!!!!\n";
    for (int i = 1; i <= q; i += 1) {
        int tp, from;
        cin >> tp >> from;
        roots[i] = roots[from];
        if (tp == 1) {
            ...
            roots[i] = incx(incx, roots[i], 1, n, -1e9, 1e9, i, prev_val, -1);
            roots[i] = incx(incx, roots[i], 1, n, -1e9, 1e9, i, val, 1);
        }
        if (tp == 2) {
            int iL, iR, valL, valR;
            cin >> iL >> iR >> valL >> valR;
            cout << gt(gt, roots[i], 1, n, -1e9, 1e9, iL, iR, valL, valR) << "\n";
        }
    }
}
