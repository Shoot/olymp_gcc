...
constexpr ll MOD7 =  993438647;
constexpr ll MOD99 = 1e9+7;
ll m=2e5;
ll k=2e5;
...
//        cin >> c[i];
    ...
    while (l <= r) {
        ...
        fo(i, 0, m+1-mid) {
            if (c_st7.find((((pref_b_7[i + mid] - pref_b_7[i] + MOD7) % MOD7) * base_rev_power7[i]) % MOD7) !=
                c_st7.end()) {
                intersected7 = true;
                break;
            }
        ...
        fo(i, 0, m+1-mid) {
            if (c_st99.find((((pref_b_99[i+mid]-pref_b_99[i]+MOD99)%MOD99)*base_rev_power99[i])%MOD99) != c_st99.end()) {
                intersected99 = true;
                break;
            }
        }
        if (intersected7 && intersected99) {
            ...
            if (intersected7 + intersected99) {
                clog << "alert" << endl;
                clog << intersected7 << intersected99 << endl;
            }
            r = mid-1;
        }
    }
